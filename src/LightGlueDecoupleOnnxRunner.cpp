...
int LightGlueDecoupleOnnxRunner::InitOrtEnv(Configuration cfg) {
  std::cout << "< - * -------- INITIAL ONNXRUNTIME ENV START -------- * ->"
            << std::endl;
  try {
    env0 = Ort::Env(ORT_LOGGING_LEVEL_WARNING,
                    "LightGlueDecoupleOnnxRunner Extractor");
    env1 = Ort::Env(ORT_LOGGING_LEVEL_WARNING,
    ...
    session_options0.SetGraphOptimizationLevel(
        GraphOptimizationLevel::ORT_ENABLE_ALL);

    session_options1 = Ort::SessionOptions();
    session_options1.SetInterOpNumThreads(std::thread::hardware_concurrency());
    session_options1.SetGraphOptimizationLevel(
        GraphOptimizationLevel::ORT_ENABLE_ALL);

    if (cfg.device == "cuda") {
      std::cout << "[INFO] OrtSessionOptions Append CUDAExecutionProvider"
                << std::endl;
...
#if _WIN32
    std::cout
        << "[INFO] Env _WIN32 change modelpath from multi byte to wide char ..."
        << std::endl;
    const wchar_t *extractor_modelPath =
        multi_Byte_To_Wide_Char(cfg.extractorPath);
    const wchar_t *matcher_modelPath =
        multi_Byte_To_Wide_Char(cfg.lightgluePath);
#else
    const char *extractor_modelPath = cfg.extractorPath.c_str();
    const char *matcher_modelPath = cfg.lightgluePath.c_str();
#endif // _WIN32

    ExtractorSession = std::make_unique<Ort::Session>(env0, extractor_modelPath,
                                                      session_options0);
    MatcherSession = std::make_unique<Ort::Session>(env1, matcher_modelPath,
                                                    session_options1);
    ...
    for (size_t i = 0; i < numInputNodes; i++) {
      ExtractorInputNodeNames.emplace_back(
          strdup(ExtractorSession->GetInputNameAllocated(i, allocator).get()));
      ExtractorInputNodeShapes.emplace_back(
          ExtractorSession->GetInputTypeInfo(i)
    ...
    for (size_t i = 0; i < numOutputNodes; i++) {
      ExtractorOutputNodeNames.emplace_back(
          strdup(ExtractorSession->GetOutputNameAllocated(i, allocator).get()));
      ExtractorOutputNodeShapes.emplace_back(
          ExtractorSession->GetOutputTypeInfo(i)
              .GetTensorTypeAndShapeInfo()
              .GetShape());
    }
    ...
    ExtractorInputNodeNames.reserve(numInputNodes);
    for (size_t i = 0; i < numInputNodes; i++) {
      MatcherInputNodeNames.emplace_back(
          strdup(MatcherSession->GetInputNameAllocated(i, allocator).get()));
      MatcherInputNodeShapes.emplace_back(MatcherSession->GetInputTypeInfo(i)
                                              .GetTensorTypeAndShapeInfo()
                                              .GetShape());
    }
    ...
    ExtractorOutputNodeNames.reserve(numOutputNodes);
    for (size_t i = 0; i < numOutputNodes; i++) {
      MatcherOutputNodeNames.emplace_back(
          strdup(MatcherSession->GetOutputNameAllocated(i, allocator).get()));
      MatcherOutputNodeShapes.emplace_back(MatcherSession->GetOutputTypeInfo(i)
                                               .GetTensorTypeAndShapeInfo()
                                               .GetShape());
    }

    std::cout << "[INFO] ONNXRuntime environment created successfully."
              << std::endl;
  } catch (const std::exception &ex) {
    std::cerr << "[ERROR] ONNXRuntime environment created failed : "
              << ex.what() << '\n';
    return EXIT_FAILURE;
  }
...
cv::Mat LightGlueDecoupleOnnxRunner::Extractor_PreProcess(Configuration cfg,
                                                          const cv::Mat &Image,
                                                          float &scale) {
  float temp_scale = scale;
  cv::Mat tempImage = Image.clone();
  std::cout << "[INFO] Image info :  width : " << Image.cols
            << " height :  " << Image.rows << std::endl;

  std::string fn = "max";
  std::string interp = "area";
  auto resultImage =
      ResizeImage(tempImage, cfg.image_size, scale, fn, interp);
  resultImage = NormalizeImage(resultImage);
  if (cfg.extractorType == "superpoint") {
    ...
    // resultImage.convertTo(resultImage, CV_32F, 1.0);
  }
  std::cout << "[INFO] Scale from " << temp_scale << " to " << scale
            << std::endl;

  return resultImage;
}

int LightGlueDecoupleOnnxRunner::Extractor_Inference(Configuration cfg,
                                                     const cv::Mat &image) {
  std::cout << "< - * -------- Extractor Inference START -------- * ->"
            << std::endl;
  try {
    // Dynamic InputNodeShapes is [1,3,-1,-1] or [1,1,-1,-1]
    std::cout << "[INFO] Image Size : " << image.size()
              << " Channels : " << image.channels() << std::endl;

    // Build src input node shape and destImage input node shape
    int srcInputTensorSize, destInputTensorSize;
    if (cfg.extractorType == "superpoint") {
      ExtractorInputNodeShapes[0] = {1, 1, image.size().height,
                                     image.size().width};
    } else if (cfg.extractorType == "disk") {
      ExtractorInputNodeShapes[0] = {1, 3, image.size().height,
                                     image.size().width};
    }
    srcInputTensorSize =
        ExtractorInputNodeShapes[0][0] * ExtractorInputNodeShapes[0][1] *
    ...
    input_tensors.push_back(Ort::Value::CreateTensor<float>(
        ...
        srcInputTensorValues.size(), ExtractorInputNodeShapes[0].data(),
        ExtractorInputNodeShapes[0].size()));

    auto time_start = std::chrono::high_resolution_clock::now();

    auto output_tensor = ExtractorSession->Run(
        Ort::RunOptions{nullptr}, ExtractorInputNodeNames.data(),
        input_tensors.data(), input_tensors.size(),
        ExtractorOutputNodeNames.data(), ExtractorOutputNodeNames.size());
    ...
    std::cout
        << "[INFO] LightGlueDecoupleOnnxRunner Extractor inference finish ..."
        << std::endl;
    std::cout << "[INFO] Extractor inference cost time : " << diff << "ms"
              << std::endl;
  } catch (const std::exception &ex) {
    std::cerr
        << "[ERROR] LightGlueDecoupleOnnxRunner Extractor inference failed : "
        << ex.what() << std::endl;
    return EXIT_FAILURE;
  }
...
LightGlueDecoupleOnnxRunner::Extractor_PostProcess(
    Configuration cfg, std::vector<Ort::Value> tensor) {
  ...
  try {
    ...
    std::cout
        << "[INFO] Extractor postprocessing operation completed successfully"
        << std::endl;
  } catch (const std::exception &ex) {
    std::cerr << "[ERROR] Extractor postprocess failed : " << ex.what()
              << std::endl;
  }
...
LightGlueDecoupleOnnxRunner::Matcher_PreProcess(std::vector<cv::Point2f> kpts,
                                                int h, int w) {
  return NormalizeKeypoints(kpts, h, w);
}

int LightGlueDecoupleOnnxRunner::Matcher_Inference(
    Configuration cfg, std::vector<cv::Point2f> kpts0,
    std::vector<cv::Point2f> kpts1, float *desc0, float *desc1) {
  std::cout << "< - * -------- Matcher Inference START -------- * ->"
            << std::endl;
  try {
    ...
    auto output_tensor = MatcherSession->Run(
        Ort::RunOptions{nullptr}, MatcherInputNodeNames.data(),
        input_tensors.data(), input_tensors.size(),
        MatcherOutputNodeNames.data(), MatcherOutputNodeNames.size());
    ...
    std::cout
        << "[INFO] LightGlueDecoupleOnnxRunner Matcher inference finish ..."
        << std::endl;
    std::cout << "[INFO] Matcher inference cost time : " << diff << "ms"
              << std::endl;
  } catch (const std::exception &ex) {
    std::cerr
        << "[ERROR] LightGlueDecoupleOnnxRunner Matcher inference failed : "
        << ex.what() << std::endl;
    return EXIT_FAILURE;
  }
...
int LightGlueDecoupleOnnxRunner::Matcher_PostProcess(
    Configuration cfg, std::vector<cv::Point2f> kpts0,
    std::vector<cv::Point2f> kpts1) {
  try {
    std::vector<int64_t> matches0_Shape =
        matcher_outputtensors[0].GetTensorTypeAndShapeInfo().GetShape();
    int64_t *matches0 =
        (int64_t *)matcher_outputtensors[0].GetTensorMutableData<void>();
    printf("[RESULT INFO] matches0 Shape : (%ld , %ld)\n", matches0_Shape[0],
           matches0_Shape[1]);

    std::vector<int64_t> matches1_Shape =
        matcher_outputtensors[1].GetTensorTypeAndShapeInfo().GetShape();
    int64_t *matches1 =
        (int64_t *)matcher_outputtensors[1].GetTensorMutableData<void>();
    printf("[RESULT INFO] matches1 Shape : (%ld , %ld)\n", matches1_Shape[0],
           matches1_Shape[1]);

    std::vector<int64_t> mscore0_Shape =
        matcher_outputtensors[2].GetTensorTypeAndShapeInfo().GetShape();
    float *mscores0 =
        (float *)matcher_outputtensors[2].GetTensorMutableData<void>();
    std::vector<int64_t> mscore1_Shape =
        matcher_outputtensors[3].GetTensorTypeAndShapeInfo().GetShape();
    float *mscores1 =
        (float *)matcher_outputtensors[3].GetTensorMutableData<void>();

    // Process kpts0 and kpts1
    std::vector<cv::Point2f> kpts0_f, kpts1_f;

    // for (int i = 0; i < kpts0.size(); i++) {
    //   kpts0_f.emplace_back(cv::Point2f((kpts0[i].x + 0.5) / scales[0] - 0.5,
    //                                    (kpts0[i].y + 0.5) / scales[0] - 0.5));
    // }
    // for (int i = 0; i < kpts1.size(); i++) {
    //   kpts1_f.emplace_back(cv::Point2f((kpts1[i].x + 0.5) / scales[1] - 0.5,
    //                                    (kpts1[i].y + 0.5) / scales[1] - 0.5));
    // }
    ...
    for (int i = 0; i < matches0_Shape[1]; ++i) {
      if (matches0[i] > -1 && mscores0[i] > this->matchThresh &&
          matches1[matches0[i]] == i) {
    ...
    std::cout << "[INFO] Postprocessing operation completed successfully"
              << std::endl;
  } catch (const std::exception &ex) {
...
LightGlueDecoupleOnnxRunner::InferenceImage(Configuration cfg,
                                            const cv::Mat &srcImage,
                                            const cv::Mat &destImage) {
  ...
  auto src_extract =
      Extractor_PostProcess(cfg, std::move(extractor_outputtensors[0]));
  auto dest_extract =
      Extractor_PostProcess(cfg, std::move(extractor_outputtensors[1]));

  // Build Matches
  auto normal_kpts0 = Matcher_PreProcess(src_extract.first, src.rows, src.cols);
  auto normal_kpts1 =
      Matcher_PreProcess(dest_extract.first, dest.rows, dest.cols);

  Matcher_Inference(cfg, normal_kpts0, normal_kpts1, src_extract.second,
                    dest_extract.second);

  // Matcher_PostProcess(cfg, src_extract.first, dest_extract.first);
  Matcher_PostProcess(cfg, normal_kpts0, normal_kpts1);
...
std::pair<std::vector<cv::Point2f>, std::vector<cv::Point2f>>
LightGlueDecoupleOnnxRunner::GetKeypointsResult() {
